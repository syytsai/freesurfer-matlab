...
%
% 230628 for windows user call matlab "gunzip" and "delete" instead of   unix "gunzip" and "rm" 
% 
...
if (strcmpi(fname((strlen(fname)-3):strlen(fname)), '.MGZ') || ...
    ...
    elseif(strcmp(computer,'PCWIN64'))  % use matlab function for winodws users
        gunzip(fname); % 
        a1=strfind(fname,'/');
        a2=strfind(fname,'.');
        [status,msg] =movefile(fname(a1(end)+1:a2(end)-1),new_fname);
    else
...
if(slices(1) <= 0 & frames(1) <= 0)
    ...
    if(gzipped >=0)
       %  [status,msg] = unix(sprintf('rm -f %s', fname));        
        %if status ~= 0, fprintf('%s\n',msg) ; end
        delete(fname); %  use matlab function for winodws users
    end
